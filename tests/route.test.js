...
// video-signal stub
vi.mock('../src/api/video-signal.js', () => ({
  VideoSignalAPI: {
    signal: (_src, _opts) => ({
...
// media-lease stub
vi.mock('../src/api/media-lease.js', () => ({
...
vi.mock('../src/api/render-pipeline.js', () => {
  const _stages = [];
...
beforeEach(async () => {
  ...
  ({ route, getLiveRoutes }            = await import('../src/api/route.js'));
  ({ liveOutput }                      = await import('../src/runtime/keep-alive.js'));
  ({ __pipelineObj: _pipelineObj, Source } = await import('../src/api/render-pipeline.js'));
