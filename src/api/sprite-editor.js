...
class SpriteFrameAdapter {
  ...
  get count() { return this.sp.frameCount; }
  get index() { return this.sp._fi; }
  get isPlaying() { return this.sp._iid != null; }
  ...
  _mutate(action) { this._events.emit('mutate', { action, index: this.sp._fi, count: this.count }); }
...
export class SpriteEditor {
  ...
  _getState() {
    ...
    return {
      title: this._title,
      width: sp._w, height: sp._h, scale: sp._scale,
      fps: this._fps,
  ...
  _floodFill(px, py) {
    ...
    sp._render();
    this._events.emit('stroke', {
      tool: 'fill', color: this._color, frame: sp._fi,
  ...
  _commitShape(start, end) {
    ...
    if (this._tool === 'line') {
      ...
      if (this._tool === 'rectfill') {
        for (let y = ty; y <= by; y++)
          for (let x = lx; x <= rx; x++) sp.pixel(x, y, color);
      } else {
        for (let x = lx; x <= rx; x++) { sp.pixel(x, ty, color); sp.pixel(x, by, color); }
  ...
  _exportCode() {
    const sp    = this.sprite;
    const lines = [`const sp = new Sprite({ width: ${sp._w}, height: ${sp._h}, scale: ${sp._scale}, frames: ${sp.frameCount} });`];
    ...
    sp._frames.forEach((fc, fi) => {
      ...
      for (let y = 0; y < sp._h; y++) {
        let runStart = -1, runColor = null, runLen = 0;
        ...
        for (let x = 0; x < sp._w; x++) {
          ...
          if (hex === runColor) {
            ...
            flush();
            runStart = x; runColor = hex; runLen = 1;
          }
        }
        flush();
      }
  ...
  onPixel(fn)  { this._events.on('pixel',  fn); return this; }
