...
export async function createProject(name = 'New Project') {
  const id = _uid();
  await _put({ id, name, createdAt: Date.now(), updatedAt: Date.now(), data: { version: 1, windows: [], desktop: [] } });
  await _syncCache();
  return id;
}
...
export async function moveEditorToProject(editorId, targetProjectId, copy = false) {
  ...
  const maxId = Math.max(0, ...proj.data.windows.filter(w => w.type === 'editor').map(w => w.editorId ?? 0));
